-- Iron City Cargo Agents — Supabase schema
...
-- Bump chats.updated_at when a message is added
...
-- profiles: users can read and update their own row (insert via auth trigger)
...
-- chats: full access to own chats
...
-- messages: access via chat ownership; user_id must match caller on insert
...
-- memory: full access to own rows
